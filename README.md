...
| プレースホルダ | 置換内容 | 出現ファイル |
| --- | --- | --- |
| `メディアツール` | ブランド名（例: お金ツール工房） | index.html, config.js, manifest.webmanifest, privacy/terms/operator.html |
| `画像・動画・音声の圧縮とQRコード作成ができる無料ツール（端末内処理）` | 説明文（meta description等） | index.html, config.js, manifest.webmanifest |
| `メディアツール｜ツール工房` | ページタイトル（`<title>` / OGP） | index.html |
...
sed -i 's/メディアツール/お金ツール工房/g; s|https://ai-kaihatsubu.github.io/media-tools/|https://example.github.io/money-suite/|g' \
